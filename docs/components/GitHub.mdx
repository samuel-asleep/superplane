...
This component requires the GitHub App to have **Organization permission: Administration (read)**.
...
- Cannot filter by specific repositories, time period, or runner OS

### Configuration

No configuration fields required - this component automatically retrieves organization-wide billing data from the GitHub App installation.
...
The GitHub billing API returns current billing cycle data only. It does not support:
- Filtering by specific repositories
- Historical data or custom time ranges
- Per-workflow breakdown (only per-OS breakdown available)
