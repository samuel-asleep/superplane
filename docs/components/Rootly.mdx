...
The On Event trigger starts a workflow execution when a Rootly incident timeline event is created or updated.

### Use Cases

- **Note tracking**: Run a workflow when someone adds a note to an incident (e.g., notify a channel, update Jira)
...
- **Filtered reactions**: Trigger actions only for specific event kinds, visibility levels, or affected services

### Configuration

All fields are optional and act as filters — if left empty the trigger fires for every timeline event.
...
- **Service**: Filter by service name or slug
- **Team**: Filter by team name or slug
...
- **id**: Event ID
- **event**: Event text content (the note or annotation)
- **kind**: Event kind (e.g., `note`, `status_change`)
...
- **incident**: Parent incident information including title, status, severity, services, and teams
...
```json
{
  "data": {
    ...
    "event": "Investigation started for API latency spike",
    "event_source": "web",
    "id": "ie-abc123-def456",
    "incident": {
      "groups": [
  ...
  "type": "rootly.incident_event.created"
}
```
